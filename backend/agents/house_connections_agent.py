...
def house_connections_context_for_narrator(natal_chart: dict) -> str:
    try:
        data = compute_house_connections(natal_chart)
    except Exception:
        return ""

    s = data["summary"]
    lines = [
        "=== HOUSE CONNECTIONS (prediction map — cite lords & blessers exactly) ===",
        f"DISCLAIMER: {DISCLAIMER_EN}",
        f"Dasa: {s['maha_dasa']} MD · {s['bhukti']} Bhukti",
        "",
        "STRONGEST HOUSES:",
    ]
    for h in s["strongest_houses"]:
        lines.append(f"  H{h['house']} {h['theme_en']} — strength {h['strength']}")

    lines.append("WEAKEST HOUSES:")
    for h in s["weakest_houses"]:
        lines.append(f"  H{h['house']} {h['theme_en']} — strength {h['strength']}")
    ...
    for pred in data["predictions"]:
        tb = pred.get("top_blesser")
        bless = tb["planet"] if tb else "—"
        lines.append(
            f"  H{pred['house']} {pred['theme_en']}: lord {pred['structure']['lord']} "
            f"in H{pred['structure']['lord_house']} · blesser {bless} · {pred['inference_en'][:120]}…"
        )
